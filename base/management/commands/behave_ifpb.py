...
from behave.configuration import options as behave_options, Configuration
...
class BehaveRunner(Runner):
    def load_step_definitions(self):
        ...
        for app in apps.get_app_configs():
            steps_dir = Path(app.path) / "features" / "steps"
            if steps_dir.exists():
                extra_step_paths.append(steps_dir)
        return super().load_step_definitions(extra_step_paths)
...
def add_command_arguments(parser):
    ...
    parser.add_argument(
        "-p", "--port", type=int, help="Port to live servver.", default=0
    )

    parser.add_argument("--host", help="Hostname to lives ervver.", default="localhost")
